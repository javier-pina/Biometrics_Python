...
class tagSAFEARRAY(ctypes.Structure):
    _fields_ = [("cDims",ctypes.c_ushort),
                ...
                ("pvData",ctypes.c_int16 * 50), # This is the array that will contain the values read from the memory buffer associated to a goniometer axis every time it is accessed using OnLineGetData function. Its size is 50 in order to ensure performance, but it can be increased
